import { MovieTypeType } from '../redux/reducers/movieReducer';
...
export const MOVIE_API_URL = async (type: MovieTypeType, page: number) => {
...
export const MOVIE_DETAILS_URL = async (id: string | number) => {
  const response = await axios.get(`${REQUEST_URL}/movie/${id}?api_key=${API_KEY}&language=en-US`);
  return response;
};

export const MOVIE_CREDITS_URL = async (id: string | number) => {
  const response = await axios.get(
...
export const MOVIE_IMAGES_URL = async (id: string | number) => {
  const response = await axios.get(
...
export const MOVIE_VIDEOS_URL = async (id: string | number) => {
  const response = await axios.get(
...
export const MOVIE_REVIEWS_URL = async (id: string | number, page = 1) => {
  const response = await axios.get(
